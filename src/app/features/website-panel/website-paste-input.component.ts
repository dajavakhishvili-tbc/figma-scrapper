...
@Component({
  selector: 'app-website-paste-input',
  template: `
    <div class="space-y-3">
      <label class="block text-sm text-text-muted">Paste website text (one item per line)</label>
      <textarea
        [value]="rawText()"
...
export class WebsitePasteInputComponent {
  ...
  extract(): void {
    const nodes = this.extractor.parsePastedAsWebsite(this.rawText());
